...
<head>
    ...
    <link rel="stylesheet" href="{{asset('css/prints/print.css')}}" />

</head>
<body>
<div id="letter-bg" style="width:100vw;height:100vh;position: fixed;top:0;left:0;">
    <img src="{{asset('css/prints/images/bg.png')}}" style="height:100%;width:100%;">
</div>
<table>
    <thead>
    <tr>
            ...
            <div class="doc-body" style="position: relative;z-index: 1009;">
                ...
                <table class="table-plain mb-4">
                    <tr>
                        ...
                        <td class="align-left">{{$invoice->number}}</td>
                        <td class="align-right">{{$invoice->number}}</td>
                        <th class="align-right">فاتورة رقم:</th>
                    </tr>
                    <tr class="border-bottom">
                        <th class="align-left">date:</th>
                        <td class="align-left">{{$invoice->created_at}}</td>
                        <td class="align-right">{{$invoice->created_at}}</td>
                    ...
                    <tr>
                        ...
                        <td class="align-left">{{ $invoice->client->name }}</td>
                        <td class="align-right">{{ $invoice->client->name }}</td>
                        <th class="align-right">اسم الشركة:</th>
                    </tr>
                    <tr>
                        <th class="align-left">street address:</th>
                        <td class="align-left">{{ $invoice->client->address()->street ?? '-' }}</td>
                        <td class="align-right">{{ $invoice->client->address()->street ?? '-' }}</td>
                        <th class="align-right">اسم الشارع:</th>
                    </tr>
                    <tr>
                        <th class="align-left">city, zip code:</th>
                        <td class="align-left">{{ $invoice->client->address()->zip_code ?? '-' }}</td>
                        <td class="align-right">{{ $invoice->client->address()->zip_code ?? '-' }}</td>
                        <th class="align-right">المدينة و الرمز البريدي:</th>
                    </tr>
                    <tr>
                        <th class="align-left">phone number:</th>
                        <td class="align-left">{{ $invoice->client->phone1 ?? '-' }}</td>
                        <td class="align-right">{{ $invoice->client->phone1 ?? '-' }}</td>
                ...
                <table class="mb-4">
                    <tr>
                        <td>sub total (السعر قبل الخصم): {{$invoice->amount}}</td>
                    </tr>
                    <tr>
